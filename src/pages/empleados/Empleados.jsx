...
const Empleados = () => {
  ...
  const GenerarPDF = () => {
    ...
    const columns = [
      ...
      { header: "Correo Electrónico", dataKey: "Mail" },
    ];
    ...
    const rows = filteredEmpleados.map(empleado => ({
      ...
      FechaIngreso: empleado.FechaIngreso.slice(0, 10), // Asumiendo que FechaIngreso es una cadena de fecha
      Telefono: empleado.Telefono,
      Mail: empleado.Mail,
    })); 
  
    // Añadir el logo con margen
    const logo = "https://img.icons8.com/external-soft-fill-juicy-fish/60/external-eco-packaging-symbols-soft-fill-soft-fill-juicy-fish.png";
    doc.addImage(logo, 'PNG', 14, 15, 20, 20); // Ajustar la posición y el tamaño del logo
  
    // Añadir información extra con margen
    doc.setFontSize(12);
    doc.text("Fecha: " + new Date().toLocaleDateString(), 50, 25); // Ajustar las coordenadas para la fecha
    doc.text("Generado por: Gestión de ecotablas", 50, 30); // Ajustar las coordenadas para el texto adicional
  
    // Añadir título del documento
    doc.setFontSize(18);
    doc.text("Listado de empleados", 14, 50); // Ajustar la posición del título
  
    // Generar la tabla con los datos
    doc.autoTable({
      head: [columns.map(col => col.header)],
      body: rows.map(row => columns.map(col => row[col.dataKey])),
      startY: 60, // Asegurar que la tabla comience después del contenido anterior
      margin: { top: 70, left: 14, right: 14 }, // Ajustar los márgenes de la tabla
      theme: 'striped'
    });
      ...
      <div className='md:flex flex-row bg-slate-900'>
        ...
        <div className="overflow-x-auto m-5">
          ...
          <div className="">
            ...
            <button onClick={GenerarPDF} className="bg-gray-800 hover:bg-gray-600 text-white font-bold py-2 mt-2 m-2 px-4 rounded">
              Imprimir listado
            </button>
